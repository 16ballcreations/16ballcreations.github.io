...
(function () {
  ...
  /* ---------- Menú en móvil ----------
     El estado inicial lo fija el CSS (.nav__links[hidden] y la media query).
     Aquí solo se alterna y se reacciona al cruce del punto de corte,
     no a cada evento de resize.                                        */
  var toggle = document.querySelector(".nav__toggle");
  var links = document.getElementById("navLinks");
  if (toggle && links) {
    var mq = window.matchMedia("(max-width:980px)");

    var apply = function (small) {
      links.hidden = small;
